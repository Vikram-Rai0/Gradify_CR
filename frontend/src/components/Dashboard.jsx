...
export default function TeacherDashboard() {
  ...
  const menuItems = [
    ...
    { label: "Students", icon: <Users size={20} /> },
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      >
        <div className="flex flex-col h-full">
          <div className="p-4 flex items-center gap-2">
            ...
            {sidebarOpen && (
              <h1 className="text-lg font-bold text-indigo-600">Gradify</h1>
            )}
          </div>
          <nav className="flex-1 p-2 space-y-1">
            {menuItems.map((item, idx) => (
              <button
                key={idx}
                className="flex items-center gap-3 w-full p-2 rounded-lg text-gray-700 hover:bg-indigo-50 hover:text-indigo-600 transition"
              >
                {item.icon}
      ...
      <div className="flex-1 p-6 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {[
            { label: "Instructor", value: 18 },
            { label: "Student", value: 42 },
            { label: "Admin", value: 1 },
          ].map((kpi, idx) => (
            <div
              key={idx}
